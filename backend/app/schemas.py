...
class IngestionRequest(BaseModel):
    ...
    content: str = Field(..., min_length=1, max_length=10000000, description="Raw log content to ingest")
...
class ParsedEvent(BaseModel):
    ...
    def validate_timestamp(cls, v):
        ...
        # Allow for reasonable clock skew (6 hours) to handle Console logs
        # that use current date with log time and may span across day boundaries
        future_limit = now + timedelta(hours=6)
        if v > future_limit:
            raise ValueError("Event timestamp cannot be more than 6 hours in the future")
        return v
